package itchat4j.core;


import com.alibaba.fastjson.JSONArray;
import com.alibaba.fastjson.JSONObject;
import itchat4j.api.MessageTools;
import itchat4j.beans.BaseMsg;
import itchat4j.face.IMsgHandlerFace;
import itchat4j.utils.enums.MsgCodeEnum;
import itchat4j.utils.enums.MsgTypeEnum;
...
public class MsgCenter {
	...
	public static JSONArray produceMsg(JSONArray msgList) {
		System.out.println(" 获取一条消息，原始数据这样的：");
		...
		for (int i = 0; i < msgList.size(); i++) {
			...
			if (m.getString("FromUserName").contains("@@") || m.getString("ToUserName").contains("@@")) { // 群聊消息
				...
				if (m.getString("Content").contains("<br/>")) {;
					...
					String actualUserName=m.getString("Content").substring(0,m.getString("Content").indexOf("<br/>")-1);
					
					m.put("Content", content);
					m.put("groupMsg", true);
			
					m.put("ActualUserName", actualUserName);
					//记录下了 ActualUserName
					System.out.println(" 记录下了 ActualUserName：");
